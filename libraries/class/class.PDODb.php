...
class PDODb
{
    ...
    private function buildInsert($tableName, $insertData, $operation)
    {
        ...
        $this->createLogs( 'insert', $tableName, $this->pdo() -> lastInsertId(),@$_SESSION[$loginAdmin]['username']);
        $this->reset();
        if ($status && $this->pdo()
            ->lastInsertId() > 0)
        {
    ...
    public function rawQuery($query, $params = null)
    {
        ...
        if (strpos($this->query, 'delete') !== false)
        {
            ...
            $this->createLogs('query', "tables", $queryLogs, @$_SESSION[$loginAdmin]['username']);
        }
        $this->reset();
        return $result;
    }
    ...
    public function update($tableName, $tableData, $numRows = null)
    {
        ...
        $this->createLogs('update', $tableName, $wherelogs, @$_SESSION[$loginAdmin]['username']);
        $this->reset();
        $this->rowCount = $stmt->rowCount();
        return $status;
    }
