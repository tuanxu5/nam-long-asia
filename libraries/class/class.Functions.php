<?php
	...
	class Functions
	{
		...
		public function changeDBName($text='')
		{
			$text = str_replace('/','_',$menu);
		...
		public function insertMenu($arrMenu){
				$arrMenu = $this->changeDBName($arrMenu);
				return $arrMenu;
			...
			echo  " <li>
			<a class= <?php if($com ==) ?> 'transition' > ".$ten." </a> 
			</li> ";
		}
		...
		public function pagination($totalq=0, $perPage=10, $page=1, $url='?')
		{
			...
			if($lastpage > 1)
			{
				$pagination .= "<ul class='flex-wrap mb-0 pagination justify-content-center'>";
		...
		public function orderStatus($status=0)
		{
	        ...
	        $str = '<select id="order_status" name="data[order_status]" class="text-sm form-control custom-select"><option value="0">Chọn tình trạng</option>';
	        foreach($row as $v)
	        {
		...
		function orderPayments()
	    {
	        ...
	        $str = '<select id="order_payment" name="order_payment" class="text-sm form-control custom-select"><option value="0">Chọn hình thức thanh toán</option>';
	        foreach($row as $v)
	        {
