<?php
	class Cart
	{
		...
		public function getColorInfo($color=0)
		{
			$str = '';
			if($color)
			{
				$row = $this->d->rawQueryOne("select namevi, photo, id, type_show, regular_price, discount, sale_price from #_color where id = ? limit 0,1",array($color));
			}
			return $row;
		}
