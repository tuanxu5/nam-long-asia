            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <?php if(isset($config['product'][$type]['checkgroup']) && count($config['product'][$type]['checkgroup']) > 0) { ?>
                        ...
                        <?php for($i=0;$i<count($items);$i++) {
                            ...
                            <tr>
                                ...
                                <td class="align-middle">
                                    ...
                                    <div class="tool-action mt-2 w-clear">
                                        <?php if(isset($config['product'][$type]['color']) && $config['product'][$type]['color'] == true) { ?>
                                            <a class="text-primary mr-3" href="index.php?com=product&act=man_color&id_product=<?=$items[$i]['id']?>&type=<?=$type?>" target="_blank" title="color"><i class="far fa-eye mr-1"></i>Color</a>
                                        <?php } ?>
                                ...
                                <?php if(isset($config['product'][$type]['checkgroup']) && count($config['product'][$type]['checkgroup']) > 0) { ?>
                                    <td class="align-middle">
                                        <div class="dropdown">
                                            <button type="button" class="btn btn-sm bg-gradient-success dropdown-toggle" id="dropdown-gallery" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Check</button>
                                            <div class="dropdown-menu" aria-labelledby="dropdown-gallery">
                                                <?php foreach($config['product'][$type]['checkgroup'] as $key=>$value) { ?>
                                                <div class="custom-control custom-control-edit">
                                                    <input type="checkbox" class="show-checkbox" id="show-checkbox-<?=$key?>-<?=$items[$i]['id']?>" data-table="product" data-id="<?=$items[$i]['id']?>" data-attr="<?=$key?>" <?=(in_array($key, $status_array)) ? 'checked' : ''?>>
                                                    <label for="show-checkbox-<?=$key?>-<?=$items[$i]['id']?>" class=""><?=$value['title_sub_photo']?></label>
                                                </div>
                                                <?php } ?>
                                            </div>
                                        </div>
