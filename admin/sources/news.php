<?php
		...
		if($id && $act!='save_copy')
		{
			...
			if($d->update('news',$data))
			{
				...
				if($func->hasFile("file1"))
				{
					...
					if($photo1 = $func->uploadImage("file1", $config['news'][$type]['img_type'], UPLOAD_NEWS, $file_name1))
					{
						$row = $d->rawQueryOne("select id, photo1 from #_news where id = ? and type = ? limit 0,1",array($id,$type));

						if(!empty($row))
						{
							$func->deleteFile(UPLOAD_NEWS.$row['photo1']);
						}

						$photoUpdate1['photo1'] = $photo1;
						$d->where('id', $id);
						$d->update('news', $photoUpdate1);
						unset($photoUpdate1);
					}
				}
				/* Photo2 */
				...
				if(isset($config['news'][$type]['schema']) && $config['news'][$type]['schema'] == true)
				{
					...
					if($buildSchema) {
						foreach($config['website']['seo'] as $k => $v) {
							//lấy url thuộc vi,en 
							if($k=='vi' || $k=='en'){
								$url_news=$config_base.$data['slug'.$k];
							}else{
								$url_news=$config_base.$data['slugvi'];
							}
							...
							$dataSchema['schema'.$k]=$func->buildSchemaArticle($id,$data['name'.$k],$url_img_news,$detail_news['date_created'],$data['date_updated'],$setting['name'.$k],$url_news,$config_base.UPLOAD_PHOTO_L.@$logo['photo'],$config_base);
						}
					}else{
				...
				if(isset($config['news'][$type]['schema']) && $config['news'][$type]['schema'] == true)
				{
					...
					if($buildSchema) {
						foreach($config['website']['seo'] as $k => $v) {
							...
							$logo = $d->rawQueryOne("select photo from #_photo where type = ? and act = ? limit 0,1",array('logo','photo_static'));
							//Tiến hành build schema article
							$dataSchema['schema'.$k]=$func->buildSchemaArticle($id_insert,$data['name'.$k],$config_base.UPLOAD_NEWS_L.$data['photo'],$data['date_created'],$data['date_created'],$setting['name'.$k],$url_news,$config_base.UPLOAD_PHOTO_L.@$logo['photo'],$config_base);
						}
					}else{
						$dataSchema = (isset($_POST['dataSchema'])) ? $_POST['dataSchema'] : null;
						if($dataSchema)
	...
	function deleteMan()
	{
		...
		{
			/* Lấy dữ liệu */
			$row = $d->rawQueryOne("select id, photo, photo1, photo2 from #_news where id = ? and type = ? limit 0,1",array($id,$type));
