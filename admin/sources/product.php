<?php
	...
	function saveMan()
	{
		global $d, $strUrl, $func, $flash, $curPage, $config, $com, $act, $type, $config_base, $setting;
		...
		if($id && $act!='save_copy')
		{
			...
			if($d->update('product',$data))
			{
				...
				if($func->hasFile("file1"))
				{
					...
					if($photo1 = $func->uploadImage("file1", $config['product'][$type]['img_type'], UPLOAD_PRODUCT, $file_name1))
					{
						$row = $d->rawQueryOne("select id, photo1 from #_product where id = ? and type = ? limit 0,1",array($id,$type));

						if(!empty($row))
						{
							$func->deleteFile(UPLOAD_PRODUCT.$row['photo1']);
						}

						$photoUpdate1['photo1'] = $photo1;
						$d->where('id', $id);
						$d->update('product', $photoUpdate1);
						unset($photoUpdate1);
					}
				}
				...
				if(isset($config['product'][$type]['schema']) && $config['product'][$type]['schema'] == true)
				{
					...
					if($buildSchema) {
						foreach($config['website']['seo'] as $k => $v) {
							...
							//lấy url thuộc vi,en 
							if($k=='vi' || $k=='en'){
								$url_pro=$config_base.$data['slug'.$k];
							}else{
								$url_pro=$config_base.$data['slugvi'];
							}
							//Kiểm tra hình ảnh
							if($data['photo']!=''){
				...
				if(isset($config['product'][$type]['schema']) && $config['product'][$type]['schema'] == true)
				{
					...
					if($buildSchema) {
						foreach($config['website']['seo'] as $k => $v) {
							...
							//Tiến hành build schema product
							$dataSchema['schema'.$k]=$func->buildSchemaProduct($id_insert,$data['name'.$k],$config_base.UPLOAD_PRODUCT_L.$data['photo'],$dataSeo['description'.$k],$data['code'],$pro_list['name'],$setting['name'.$k],$url_pro,$data['regular_price']);
						}
					}else{
						$dataSchema = (isset($_POST['dataSchema'])) ? $_POST['dataSchema'] : null;
						if($dataSchema)
	...
	function deleteMan()
	{
		...
		{
			/* Lấy dữ liệu */
			$row = $d->rawQueryOne("select id, photo, photo1, photo2 from #_product where id = ? and type = ? limit 0,1",array($id,$type));
	...
	function editSize()
	{
		...
		if(isset($_REQUEST['id_product']))
		{
			$id_product = "&id_product=".$_REQUEST['id_product'];
		}

		if(empty($id))
		{
			$func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_size&type=".$type.$id_product."&p=".$curPage, false);
		}
		else
		{
			...
			if(empty($item))
			{
				$func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_size&type=".$type.$id_product."&p=".$curPage, false);
			}
		}
	}
	...
	function saveSize()
	{
		...
		if(!empty($response))
		{
			...
			if($id)
			{
				$func->redirect("index.php?com=product&act=edit_size&type=".$type.$id_product."&p=".$curPage."&id=".$id);
			}
			else
			{
				$func->redirect("index.php?com=product&act=add_size&type=".$type.$id_product."&p=".$curPage);
			}
		}
		...
		if($id)
		{
			...
			{
				$func->transfer("Cập nhật dữ liệu thành công", "index.php?com=product&act=man_size&type=".$type.$id_product."&p=".$curPage);
			}
			else
			{
				$func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_size&type=".$type.$id_product."&p=".$curPage, false);
			}
		}
		else
		{
			...
			{
				$func->transfer("Lưu dữ liệu thành công", "index.php?com=product&act=man_size&type=".$type.$id_product."&p=".$curPage);
			}
			else
			{
				$func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_size&type=".$type.$id_product."&p=".$curPage, false);
			}
		}
	}
	...
	function editColor()
	{
		...
		if(empty($id))
		{
			$func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_color&type=".$type.$id_product."&p=".$curPage, false);
		}
		else
		{
			...
			if(empty($item))
			{
				$func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_color&type=".$type.$id_product."&p=".$curPage, false);
			}
		}
	}
	...
	function saveColor()
	{
		...
		if(!empty($response))
		{
			...
			if($id)
			{
				$func->redirect("index.php?com=product&act=edit_color&type=".$type.$id_product."&p=".$curPage."&id=".$id);
			}
			else
			{

				$func->redirect("index.php?com=product&act=add_color&type=".$type.$id_product."&p=".$curPage);
			}
		}
		...
		if($id)
		{
			...
			if($d->update('color',$data))
			{
				...
				$func->transfer("Cập nhật dữ liệu thành công", "index.php?com=product&act=man_color&type=".$type.$id_product."&p=".$curPage);
			}
			else
			{
				$func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_color&type=".$type.$id_product."&p=".$curPage, false);
			}
		}
		else
		{
			...
			if($d->insert('color',$data))
			{
				...
				$func->transfer("Lưu dữ liệu thành công", "index.php?com=product&act=man_color&type=".$type.$id_product."&p=".$curPage);
			}
			else
			{
				$func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_color&type=".$type.$id_product."&p=".$curPage, false);
			}
		}
	}
		...
		if($id)
		{
			...
			if($d->update('product_list',$data))
			{
				...
				if($func->hasFile("file1"))
				{
					...
					if($photo1 = $func->uploadImage("file1", $config['product'][$type]['img_type_list'], UPLOAD_PRODUCT, $file_name1))
					{
						$row = $d->rawQueryOne("select id, photo1 from #_product_list where id = ? and type = ? limit 0,1",array($id,$type));
						...
						$photoUpdate1['photo1'] = $photo1;
						$d->where('id', $id);
						$d->update('product_list', $photoUpdate1);
						unset($photoUpdate1);
					}
				}
				if($func->hasFile("file2"))
				{
					...
					if($photo2 = $func->uploadImage("file2", $config['product'][$type]['img_type_list'], UPLOAD_PRODUCT, $file_name2))
					{
						...
						$photoUpdate2['photo2'] = $photo2;
						$d->where('id', $id);
						$d->update('product_list', $photoUpdate2);
						unset($photoUpdate2);
					}
				}
	...
	function deleteList()
	{
		...
		{
			/* Lấy dữ liệu */
			$row = $d->rawQueryOne("select id, photo,photo1,photo2 from #_product_list where id = ? and type = ? limit 0,1",array($id,$type));
