...
function FirstLoadAPIStatus(div,url,name_api){
    $(div).addClass('active');
    var status = $(div).data('status');
    var tenkhongdau = $(div).data('tenkhongdau');
    FrameAjaxStatus(url,"POST",{status:status,tenkhongdau:tenkhongdau},name_api);
}

function LoadAPIStatus(div,url,name_api){
...
function FrameAjaxStatus(url,type,data,name){
    $.ajax({
        url: url,
        type: type,
        data: data,
        success:function(data){
            $(name).html(data);
            NN_FRAMEWORK.Lazys();
            NN_FRAMEWORK.OwlData($('.owl-pronb'));
        }
    });
}
// End Load Ajax Status
