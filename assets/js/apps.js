...
NN_FRAMEWORK.Cart = function(){
    if (isExist($(".themgiohang"))) {
        $("body").on("click", ".themgiohang", function () {
          $(".themgiohang").removeClass("chongclick");
          var linkhinh = $(this)
            .parents(".box-product")
            .find(".pic-product img")
            .attr("data-src");
          var parent = $(this).parents(".box-product");
          var cart = $(document).find(".cart-fixed");
          $("<img />", {
            class: "fly-to-cart",
            src: linkhinh,
          })
            .appendTo("body")
            .css({
              left: parent.offset().left + parent.width() / 2 - 25,
              top: parent.offset().top + parent.height() / 2 - 25,
            });
          setTimeout(function () {
            $(".fly-to-cart").css({
            ...
            setTimeout(function () {
              $(document).find(".fly-to-cart").remove();
            }, 1000);
          }, 500);
        });
  }
...
// NN_FRAMEWORK.load_ajax_status = function(){
//     if (isExist($('.pronb-status'))) {
//         FirstLoadAPIStatus('.pronb-status:first','api/load_ajax_status.php','.load_ajax_status');
//         LoadAPIStatus('.pronb-status','api/load_ajax_status.php','.load_ajax_status');
//     }
// };

/* Ready */
$(document).ready(function(){
    // NN_FRAMEWORK.load_ajax_status();
    NN_FRAMEWORK.Homes();
